...
const ARWEAVE_GQL_ENDPOINTS = parseEndpointList(process.env.SHARME_ARWEAVE_GQLS, DEFAULT_GQL_ENDPOINTS);
const ARWEAVE_DATA_ENDPOINTS = parseEndpointList(process.env.SHARME_ARWEAVE_DATAS, DEFAULT_DATA_ENDPOINTS);
 ...
 */
export async function queryShards(walletAddress) {
    const shards = [];
  ...
  `;
    while (true) {
        ...
        if (pageCount > GQL_MAX_PAGES) {
            throw new Error(`Arweave GraphQL pagination exceeded ${GQL_MAX_PAGES} pages.`);
        }
 ...
 */
export async function queryConversationChunks(walletAddress) {
    const chunks = [];
    const seenTxIds = new Set();
    let after = null;
    let pageCount = 0;
            ...
            if (typeof maxBytes === "number") {
                ...
                if (contentLength) {
                    ...
                    if (Number.isFinite(declared) && declared > maxBytes) {
                        throw new Error(`blob too large: ${declared} bytes (max ${maxBytes})`);
                    }
                }
            }
            const buffer = await res.arrayBuffer();
            if (typeof maxBytes === "number" && buffer.byteLength > maxBytes) {
                throw new Error(`blob too large: ${buffer.byteLength} bytes (max ${maxBytes})`);
            }
...
function parseEndpointList(envValue, fallback) {
...
async function gqlRequest(body) {
    ...
    throw new Error(`Arweave GraphQL request failed across all gateways: ${errors.join(" | ")}`);
}
