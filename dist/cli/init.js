...
export async function initCommand() {
    ...
    const answer = await prompt(`  Type word ${idx1 + 1} and ${idx2 + 1} to confirm: `);
...
export async function initExistingCommand() {
    ...
    try {
        ...
        if (!identity) {
            throw new Error("Recovery failed: identity transaction not found on Arweave.");
        }
        ...
        if (Buffer.compare(Buffer.from(recoveredPrivateKey), Buffer.from(keypair.privateKey)) !== 0) {
            throw new Error("Recovered identity does not match the provided recovery phrase.");
        }
